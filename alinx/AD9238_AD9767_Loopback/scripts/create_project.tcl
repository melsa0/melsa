##############################################################################
## AD9767 -> AD9238 Loopback Test - Vivado Build Script
...
##   3. Create and configure clk_wiz_0 IP
...
puts "============================================"
puts " AD9767-AD9238 Loopback Project Builder"
...
add_files -norecurse [list \
    "$project_dir/src/loopback_top.v" \
    "$project_dir/src/waveform_display.v" \
    "$project_dir/src/hdmi_tx.v" \
    "$project_dir/src/sin1024.mem" \
]

# Set top module
set_property top loopback_top [current_fileset]

# =========================================================================
# Step 3: Create and Configure clk_wiz_0 IP (MMCM)
# Input:  50 MHz single-ended
# Output: 74.25 MHz (pixel), 371.25 MHz (5x serial), 65 MHz (ADC/DAC)
# =========================================================================
...
add_files -fileset constrs_1 -norecurse "$project_dir/constraints/loopback_pins.xdc"
...
if {$synth_status != "synth_design Complete!"} {
    ...
    # Open synthesis report for review
    open_run synth_1
...
set bit_file "$proj_path/${proj_name}.runs/impl_1/loopback_top.bit"
...
if {[file exists $bit_file]} {
    file copy -force $bit_file "$output_dir/loopback_top.bit"
    puts "\n============================================"
    puts " BUILD SUCCESSFUL!"
    puts " Bitstream: $output_dir/loopback_top.bit"
    puts "============================================"
} else {
    ...
    if {[llength $alt_bit] > 0} {
        ...
        file copy -force $actual_bit "$output_dir/loopback_top.bit"
        puts " Found bitstream at: $actual_bit"
        puts " Copied to: $output_dir/loopback_top.bit"
        puts "============================================"
    } else {
...
puts "    open_hw_target"
puts "    set_property PROGRAM.FILE {$output_dir/loopback_top.bit} [current_hw_device]"
